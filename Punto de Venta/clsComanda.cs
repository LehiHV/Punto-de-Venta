...
namespace Punto_de_Venta
{
    internal class clsComanda
    {
        public class CreaComanda
        {
            ...
            {
                string LineEncavesado = "Cant         Articulo         ";   // agrega lineas de  encabezados
            ...
            public void AgregaArticulo(List<string> art)
            {
                for (int j = 0; j < art.ToArray().Length; j++)
                {
                    string Articulo;
                    int cant;
                    Articulo = Convert.ToString(art[j].Substring(3));
                    cant = Convert.ToInt32(Convert.ToInt32(art[j].Substring(0, art[j].IndexOf("x"))));
                    
                    line.AppendLine(cant + "x  " +Articulo);
                }
            }
