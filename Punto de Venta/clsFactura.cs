...
namespace Punto_de_Venta
{
    internal class clsFactura
    {
        public class CreaTicket
        {
            ...
            {
                string LineEncavesado = "Articulo         Cant    Valor";   // agrega lineas de  encabezados
            ...
            public void TextoCentro(string par1)
            {
                ...
                max = (int)(33 - parte1.Length) / 2;         // saca la cantidad de espacios libres y divide entre dos
                for (int i = 0; i < max; i++)                // **********
                {
            ...
            public void AgregaTotales(string par1, double total)
            {
                ...
                max = 30 - (parte1.Length + parte2.Length);
                for (int i = 0; i < max; i++)                // **********
                {
            ...
            public void AgregaArticulo(List<string> art)
            {
                for (int j = 0; j < art.ToArray().Length; j++)
                {
                    string Articulo;
                    //decimal precio;
                    int cant;
                    double subtotal;
                    Articulo = Convert.ToString(art[j].Substring(3, art[j].IndexOf(":") - 3));
                    //precio = Convert.ToInt32(Convert.ToDecimal(art[j].Substring(art[j].IndexOf(":") + 1)) / Convert.ToDecimal(art[j].Substring(0, art[j].IndexOf("x"))));
                    cant = Convert.ToInt32(Convert.ToInt32(art[j].Substring(0, art[j].IndexOf("x"))));
                    subtotal = Convert.ToDouble(Convert.ToDecimal(art[j].Substring(art[j].IndexOf(":") + 1)));
